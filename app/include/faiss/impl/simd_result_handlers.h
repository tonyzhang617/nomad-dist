...
/** Structure that collects results as-is */
template <class C, bool with_id_map = false>
struct SimpleHandler : SIMDResultHandler<C, with_id_map> {
    using T = typename C::T;
    using TI = typename C::TI;

    ALIGNED(32) uint16_t d32tab[32];
    TI id_offset, ntotal;
    const MetricType _metric_type;

    SimpleHandler(size_t id, size_t n_total, MetricType metric_type = MetricType::METRIC_INNER_PRODUCT)
            : SIMDResultHandler<C, with_id_map>(32), id_offset(id), ntotal(n_total), _metric_type(metric_type) {}
    ...
    void to_flat_arrays(
        ...
        float one_a = 1.0, b = 0.0;
        if (normalizers) {
            one_a = 1 / normalizers[0];
            b = normalizers[1];
        }
        TI n_valid = ntotal - id_offset;
        if (_metric_type == MetricType::METRIC_INNER_PRODUCT) {
            for (int i = 0; i < (32 < n_valid ? 32 : n_valid); ++i) {
                distances[i] = -(d32tab[i] * one_a + b);
                labels[i] = i + id_offset;
            }
        } else {
            for (int i = 0; i < (32 < n_valid ? 32 : n_valid); ++i) {
                distances[i] = d32tab[i] * one_a + b;
...
/** Handler that stores results sequentially as-is */
template <class C, bool with_id_map = false>
struct SequenceHandler : SIMDResultHandler<C, with_id_map> {
    using T = typename C::T;
    using TI = typename C::TI;

    size_t nq, ntotal2; // ntotal2 is ntotal rounded up to a multiple of 32
    AlignedTable<uint16_t> all_vals;

    SequenceHandler(size_t nq, size_t ntotal, size_t ntotal2)
            : SIMDResultHandler<C, with_id_map>(ntotal), nq(nq), ntotal2(ntotal2), all_vals(nq * ntotal2) {}

    void handle(size_t q, size_t b, simd16uint16 d0, simd16uint16 d1) {
        d0.store(all_vals.data() + q * this->ntotal2 + this->j0);
        d1.store(all_vals.data() + q * this->ntotal2 + this->j0 + 16);
    }
    ...
    void to_flat_arrays(
            ...
            const float* normalizers = nullptr) override {
        for (int q = 0; q < nq; q++) {
            float * dis_dst = distances + q * ntotal2;
            uint16_t * dis_src = all_vals.data() + q * ntotal2;
